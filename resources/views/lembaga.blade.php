    ...
    <div class="card">
      @foreach ($lembaga as $item)
        <div class="card-header d-flex justify-content-between">
          ...
          <div>
            <button type="submit" class="btn btn-primary btn-icon" data-bs-toggle="modal" data-bs-target="#editlembaga{{ $item->id }}">
        ...
        <div class="card-body">
          ...
          <p><a href="#" class="text-body" target="_blank">{{ $item->alamat == null ? '-' :  $item->alamat }}</a></p>
          </div>
          <div class="mt-2">
          <h6 class="mb-1">Kepala:</h6>
          <p><a href="#" class="text-body">{{ $item->nama_ketua == null ? '-' :  $item->nama_ketua }}</a></p>
          </div>
        </div>
          <!-- Modal untuk Edit Lembaga -->
          <div class="modal fade" id="editlembaga{{ $item->id  }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="editUnitKerjaModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form class="row g-3 needs-validation" method="POST" action="/lembaga/update/{{ $item->id }}" novalidate>
                            @csrf
                            <input type="hidden" name="unit_id" id="editUnitId">
                            <div class="col-md-12 position-relative">
                                <label for="editNamaUnitKerja" class="form-label">Kabupaten / Kota</label>
                                <input type="text" class="form-control" name="kabupaten" id="" value="{{ $item->kabupaten }}" required>
                            </div>
                            <div class="col-md-12 position-relative">
                              <label for="editNamaUnitKerja" class="form-label">Telp</label>
                              <input type="text" class="form-control" name="telp" id="" value="{{ $item->telp }}"required>
                            </div>
                            <div class="col-md-12 position-relative">
                              <label for="editNamaUnitKerja" class="form-label">Email</label>
                              <input type="text" class="form-control" name="email" id="" value="{{ $item->email }}" required>
                            </div>
                            <div class="col-md-12 position-relative">
                            ...
                            <div class="col-md-12 position-relative">
                              ...
                              <input type="text" class="form-control" name="nama_ketua" id="" value="{{ $item->nama_ketua }}" required>
                            </div>
